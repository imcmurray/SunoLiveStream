...
export function chooseBroadcast(activeItems = [], upcomingItems = []) {
  // Among several on-air broadcasts, prefer the most-recently-started — if an
  // old auto/test broadcast lingers in "active" alongside a fresh one, we lock
  // onto the new one instead of whatever the API happens to return first.
  const onAir = (activeItems || []).filter((b) => b.snippet?.liveChatId)
    .sort((a, b) => new Date(b.snippet.actualStartTime || 0) - new Date(a.snippet.actualStartTime || 0))[0];
...
export async function* youtubeSource() {
  ...
  for (;;) {
    ...
    try {
      ...
      if (res.status === 401) { token = await getTokenWithRetry(true); res = await fetchT(url, { headers: { Authorization: `Bearer ${token}` } }); await bill(CHAT_UNITS); }
